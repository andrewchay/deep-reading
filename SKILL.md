...
⚠️ **Progressive disclosure**: the technical main-report template, technical module templates, 6-phase workflow, WikiLink creation rules, link validation checklist, and a complete worked example live in `references/technical-mode.md`. **READ that file before starting any technical reading** — do not reconstruct templates from memory.
...
⚠️ **Progressive disclosure**: the full 6-phase technical workflow, WikiLink creation rules, and link validation checklist are in `references/technical-mode.md` — READ it before starting technical reading. The governing rule: **all `[[...]]` link targets must exist before the link is written; create leaf nodes (modules, concept/author pages) before the root (main report)**.
...
If not exists, create from the template in `references/kb-templates.md` → Author Page Templates (中文/English).
...
Templates (中文/English): `references/kb-templates.md` → Technical Concept Page Templates.
...
Templates (中文/English): `references/kb-templates.md` → Domain MOC Templates.
...
⚠️ **Progressive disclosure**: the full checklist (pre-creation, during creation, post-creation verification, common mistakes, emergency fix) is in `references/technical-mode.md` → Link Validation Checklist. The one invariant: **never leave a `[[...]]` pointing to a non-existent file**.
...
A complete end-to-end technical reading walkthrough — Brad Efron's bootstrap slides, with the correct leaf-first creation order — is in `references/technical-mode.md` → Worked Example.
